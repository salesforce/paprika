...
class Task_Head(nn.Module):
    def __init__(self, args, logger):
        ...
        #### embedding layers
        self.cls_embed_layer = nn.Embedding(1, args.model_step_cls_segment_hidden_dim)
        
        if args.model_step_cls_time_pos_embed_type == 'absolute_learned_1D':
        ...
        if self.args.model_step_cls_head_name == 'downstream_transformer':
            self.long_term_model = TransformerEncoderLayer(
                args.model_step_cls_segment_hidden_dim, 
                args.model_step_cls_tx_nhead, 
                args.model_step_cls_tx_dim_feedforward,
                args.model_step_cls_tx_dropout, 
                args.model_step_cls_tx_activation)
        
        self.classifier = build_mlp(
            input_dim=args.model_step_cls_segment_hidden_dim, 
            hidden_dims=[args.model_step_cls_classifier_hidden_dim], 
            output_dim=args.model_step_cls_num_classes)
    ...
    def forward(self, video_feats, video_mask):
        ...
        if self.args.model_step_cls_time_pos_embed_type == 'absolute_learned_1D':
            time_ids = torch.arange(1, T+1, device=device).repeat(B, 1)
            time_seq = self.time_embed_layer(time_ids) 
        elif self.args.model_step_cls_time_pos_embed_type == 'fixed_sinusoidal_1D':
        ...
        elif self.args.model_step_cls_head_name == 'downstream_mlp':
